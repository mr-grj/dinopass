...
main() {
  ...
  if [ -f .env ]; then
    ...
    if docker volume inspect ciphermoth_postgres_data >/dev/null 2>&1; then
      warn "Found an existing CipherMoth database, but this folder has no .env."
      warn "Postgres keeps its original password, so a fresh random one can't unlock it."
      if ask_yes_no "Erase the old vault and start completely fresh? [y/N]" "${CIPHERMOTH_WIPE:-N}"; then
        POSTGRES_PASSWORD=unused docker compose -f docker-compose.prod.yml \
          --profile autoupdate down -v >/dev/null 2>&1 || true
        say "Old database erased."
      else
        warn "Keeping it. Put the original POSTGRES_PASSWORD in a .env here and re-run."
        die "Stopped so you don't get locked out of an existing vault."
      fi
    fi
    _pw="$(gen_password)"
    cat >.env <<EOF
