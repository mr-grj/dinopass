...
- **Secure notes.** Store free-form encrypted text (recovery codes, Wi-Fi credentials, passport details, anything) as a first-class item alongside logins. Pick "Secure note" when adding an entry and the form adapts to a single large note field; notes are encrypted at rest like everything else, can carry folders, tags, and custom fields, and are skipped by password-specific features (2FA, strength, and the Vault Health report).
